...
class Flexrest extends oauth_client_class
{
    protected $ci;
    ...
    public function getItemMetadata ($uuid, $version, &$response)
    {
        $url = $this->institute_url . 'api/item/' . $uuid . '/'. $version . '?info=metadata' ;
	...
	public function reject_item($item_uuid, $item_version, $task_uuid, $msg, &$response)
	{
		$url = $this->institute_url . 'api/item/' . $item_uuid. '/'. $item_version.'/task/' . $task_uuid . '/reject?message=' . $msg;
		return($this->post($url, $parameters=null, $options=null, $response));
	}
    ...
    public function editLockedItemAttachment($uuid, $version, $item_bean, $lock_uuid, &$response, $filearea_uuid=null)
    {
          $url = $this->institute_url . 'api/item/' . $uuid . '/'. $version.'?lock='.$lock_uuid.'&keeplocked=true&waitforindex=0';
          if ($filearea_uuid != null)
            $url .= '&file=' . $filearea_uuid;
