...
class Maptest extends CI_Controller {
	...
	{	
		...
		$course=array();
		$data = array('course' => $this->recursiveCall(4,'MD','','MMED8', $collection_uuid,'TAA', $course, 1));
		unset($course);
	...
	****************************************/
	private function getActivityRecursiveCall($attatchmentUuid, $uuid, $version, $topicCode, $do_condition, $itemresponse, $index, &$keys, &$data, $furtherCall)
	{
						...
						foreach($attachments as $attachment)
						{
							if($attachment['type'] == 'linked-resource')
							{
								if($linked_uuid_object['uuid'] == $attachment['uuid'])
								{
									...
									$getItem = $this->flexrest->getItem($uuid, $version,$iresponse);
									if(!$getItem)
									{
										log_message('error', '#297 maptest getActivitiesRecursiveCall() - activity item not found: '.$uuid.'/'. $version);
										return;
									}
									else
									{
		...
		{
			 //echo 'item not found';
			log_message('error', '#336 maptest getActivitiesRecursiveCall() - activity item not found: '.$uuid.'/'. $version. '/'.$itemresponse);
			return;
		}
		/*echo "<pre>";                                 
		print_r($data);
