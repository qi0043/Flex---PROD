				...
				if ($Uuid == $aUuid) {  // uuid match
					...
					switch ($attachment['type']) {
						...
						case 'linked-resource':
							...
							// get more information about the attachment
							
							
							$itemsuccess = $this->flexrest->getItemAttachments($attachment['itemUuid'], $attachment['itemVersion'], $itemresponse);
							
							$item_array['overall']['linked'][$l]['filename'] = $itemresponse['attachments'][0]['filename'];
							$item_array['overall']['linked'][$l]['aUuid'] = $itemresponse['attachments'][0]['uuid'];
							$item_array['overall']['linked'][$l]['type'] = $itemresponse['attachments'][0]['type'];
							 }
							 */
       ...
       // $collections = '6704afea-e88c-4230-b277-6d9d413bfbff';  // The Topic collection
		...
		foreach ($taaresponse['attachments'] as $relatedItem) {
		 ...
		 // $sysID = $item_array['sysUUID'];
			...
			$data = array('item' => $item_array, 'activity_level' => $activity_level, 'activity_los' => $activitylos_array, 'los' => $lo_array, 'token' => $this->generateToken($fan), 'privilege' => $user_role );
