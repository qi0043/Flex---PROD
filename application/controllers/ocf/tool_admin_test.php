...
class Tool_admin_test extends CI_Controller 
{
	protected $update_status;
	protected $activities_csv;
	protected $activities_upd_csv;
	protected $activities_ulk_csv;
	...
	{	
                ...
                $this->activities_csv = array();
		$file_path = 'application/controllers/ocf/temp/learning_activities_20160826_1.csv';
   	        $row = 1;
          	if (($handle = fopen($file_path, "r")) !== FALSE) {
	            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			$num = count($data);
			$data[$num] = "";
			$this->activities_csv[] = $data;
		...
		$data = array('course' => $this->recursiveCall($course_year, $courseCode,'','', $collection_uuid,'TAA', $course, 1, $error_array=array()));
                ...
                $fp = fopen('application/controllers/ocf/temp/learning_activities_20160826_result1.csv', 'cw');

                   foreach ($this->activities_csv as $fields) {
                   fputcsv($fp, $fields);
                }
	...
	****************************************/
	private function getActivityRecursiveCall($attatchmentUuid, $uuid, $version, $topicCode, $do_condition, $itemresponse, $index, &$keys, &$data, $furtherCall)
	{
		...
		$found1 = false;
		for($i=0; $i<count($this->activities_csv); $i++)
		{
			$line1 = $this->activities_csv[$i];
			if($uuid == $line1[1] && $version == $line1[2])
			{
				$found1 = true;
				$this->activities_csv[$i][5] = 'Tree';
				break;
			}

		}
		if($found1 == false)
			echo '<br><b>Item not found in csv: uuid: '.$uuid.'</b>';
    ...
    public function update_12975793()
    {
	...
	$this->activities_upd_csv = array();
	$file_path = 'application/controllers/ocf/temp/learning_activities_20160826_result1_tree2www.csv';
	#$row = 1;
	if (($handle = fopen($file_path, "r")) !== FALSE) {
	    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
		$num = count($data);
		$data[$num] = "";
		$this->activities_upd_csv[] = $data;
	...
	for($i=0; $i<count($this->activities_upd_csv); $i++)
	{
		$line1 = $this->activities_upd_csv[$i];
		$uuid = $line1[0];
		$version = 1;
		...
		echo "</pre>";
		 */
		#$this->load->library('xmlwrapper/xmlwrapper', array('xmlString' => (string)$item_bean['metadata']));
		$xmlwrapper_name = 'xmlwrapper'.$i;
                $this->load->library('xmlwrapper/xmlwrapper', array('xmlString' => (string)$item_bean['metadata']),$xmlwrapper_name);
		
		$xpath_management_id = '/xml/item/restrictions/management/manage';
		$node_management_id = $this->$xmlwrapper_name->createNodeFromXPath($xpath_management_id);
		
		$tmp_mgmt_code_value = $this->$xmlwrapper_name->nodeValue('/xml/item/restrictions/management/manage/@ocf_id');
		$msg = 'Success';
	        if($tmp_mgmt_code_value != 'MD')
		{
		    $node_management_id_ocf_id = $this->$xmlwrapper_name->createAttribute($node_management_id, "ocf_id");
                    $node_management_id_ocf_id->nodeValue = 'MD';
		
		    $item_bean['metadata'] = $this->$xmlwrapper_name->__toString();
		    $success = $this->flexrest->editItem($uuid, $version, $item_bean, $response3);
		    if(!$success)
		    {
			$errdata['message'] = $this->flexrest->error;
			...
			#echo $errdata['message'];
			#return;
			$msg = 'Failed: '. $errdata['message'];
		    }

		}
		else
		{
			$msg = 'Already Set to MD';
                }
		unset($this->$xmlwrapper_name);
		$this->activities_upd_csv[$i][1] = $msg;
		echo 'Item: '. ($i+1) . '  ' . $uuid. '  ' . $msg.'<br>';

	}
	
	$fp = fopen('application/controllers/ocf/temp/learning_activities_20160826_result1_tree2_result1.csv', 'cw');

	foreach ($this->activities_upd_csv as $fields) {
	   fputcsv($fp, $fields);
	}
    ...
    public function unlock_12975793()
    {
	$this->load->library('flexrest/flexrest');
	$success = $this->flexrest->processClientCredentialToken();
	if(!$success)
	...
	$this->activities_ulk_csv = array();
	$file_path = 'application/controllers/ocf/temp/unlock_20160829_1www.csv';
	#$row = 1;
	if (($handle = fopen($file_path, "r")) !== FALSE) {
	    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
		$num = count($data);
		$data[$num] = "";
		$this->activities_ulk_csv[] = $data;
	...
	for($i=0; $i<count($this->activities_ulk_csv); $i++)
	{
		$line1 = $this->activities_ulk_csv[$i];
		$uuid = $line1[0];
		$version = 1;
		...
		$success = $this->flexrest->deleteLock($uuid, $version, $response2);
		if(!$success)
		{
		    $errdata['message'] = $this->flexrest->error;
		    echo 'Failed: '. ($i+1). '  ' . $uuid. '  '. $errdata['message'];
		    #log_message('error', 'Failed to get ereading list mgt item' . ', uuid: ' . $uuid . ', error: ' . $errdata['message']);
		    #$this->load->view('reading_listmgr/showerror_view', $errdata);
		    return;
		}
		...
		#$this->activities_ulk_csv[$i][1] = 'Success';
		echo 'Unlocked: '. ($i+1) . '  ' . $uuid.'<br>';

	}
	
	/*$fp = fopen('application/controllers/ocf/temp/unlock_20160826_test1_result1.csv', 'cw');

	foreach ($this->activities_ulk_csv as $fields) {
	   fputcsv($fp, $fields);
	}
    ...
    public function nontree_12975793()
    {
	ini_set('max_execution_time', 0);
	$this->load->library('flexrest/flexrest');
	$success = $this->flexrest->processClientCredentialToken();
	...
	$this->activities_upd_csv = array();
	$file_path = 'application/controllers/ocf/temp/learning_activities_20160826_result1_nontre2www.csv';
	#$row = 1;
	if (($handle = fopen($file_path, "r")) !== FALSE) {
	    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
		$num = count($data);
	...
	for($i=0; $i<count($this->activities_upd_csv); $i++)
	{
		...
		$tmp_mgmt_code_value = $this->$xmlwrapper_name->nodeValue('/xml/item/restrictions/management/manage/@ocf_id');
		$msg = 'Success';
	        if($tmp_mgmt_code_value == '' || $tmp_mgmt_code_value == null)
		{
		    $node_management_id_ocf_id = $this->$xmlwrapper_name->createAttribute($node_management_id, "ocf_id");
                    $node_management_id_ocf_id->nodeValue = 'md-error';
	...
	$fp = fopen('application/controllers/ocf/temp/learning_activities_20160826_result1_nontre2_result1.csv', 'cw');
