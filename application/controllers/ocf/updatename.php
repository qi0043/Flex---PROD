...
class Updatename extends CI_Controller {
	...
	public function index($uuid='missed', $version='missed')
	{
		...
		if(!(isset($upd_privilege['has_privilege']) && $upd_privilege['has_privilege'] == 'Yes' && $upd_privilege['locks']['item_name'] != 'Yes'))
		{
		    echo 'Error: no privilege or item name locked.';
		    return;
		}
		...
		{		
		   ...
		   {
				$errdata['message'] = $this->flexrest->error;
				log_message('error', 'Activity name update fail, item uuid: ' . $uuid . ', error: ' . $errdata['message']);
		   ...
		   else
		   {
			...
			if(!$return_topics)
			{
				$errdata['message'] = 'have not found this activity';
				return;
			}
				...
				for($x=0; $x<$length; $x++)
				{
					...
					if(stripos($href, 'ocf/lta/'.$uuid_version)>0) //activity group
					{
						...
						{ 
							...
							{
								$count = $xpath->query(".//small", $prev->item(0))->length;
								if($count > 0)
								{
									...
									if(strrpos($path, "/a") >1)
									{
										$x_xpath = substr($path, 0, strrpos($path, "/a")).'/span/small';
										//log_message('error', $x_xpath);
									}	
																	
									$spath = new DOMXPath($dom);
									$s = $spath->evaluate($x_xpath);
									$s_value = $s->item(0)->textContent;
									//log_message('error', $s_value);
									if($child_name.$s_value == $old_name)
									{
										$spos= stripos($old_name, $s_value);
