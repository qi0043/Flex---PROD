...
class Activityflo extends CI_Controller {
				...
				if ($Uuid == $aUuid) {
					...
					switch ($attachment['type']) {
						...
						case 'file':
							...
							$explodedfile = explode('.', $attachment['filename']);
							$item_array['overall']['instructions']['filetype'] = $explodedfile[1];
				...
				if ($Uuid == $aUuid) {  // uuid match
					...
					switch ($attachment['type']) {
						...
						case 'file':
							...
							$explodedfile = explode('.', $attachment['filename']);
							$item_array['overall']['linked'][$l]['filetype'] = $explodedfile[1];
				...
				if ($Uuid == $aUuid) {
					...
					switch ($attachment['type']) {
						...
						case 'file':
							...
							$explodedfile = explode('.', $attachment['filename']);
							$item_array['pre']['instructions']['filetype'] = $explodedfile[1];
				...
				if ($Uuid == $aUuid) {  // uuid match
					...
					switch ($attachment['type']) {
						...
						case 'file':
							...
							$explodedfile = explode('.', $attachment['filename']);
							$item_array['pre']['linked'][$l]['filetype'] = $explodedfile[1];
...
// get infor for linked resources
		...
		*/ 		 
		...
		$this->load->view('ocf/activity_itemFLO', $data);
