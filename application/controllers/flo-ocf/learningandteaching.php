...
* @see http://codeigniter.com/user_guide/general/urls.html
	**/
	...
	{	
		...
		$userUuid = strtolower($_SESSION['flo_fan']);
		...
		$upd_privilege = $this->permission->get_ocf_activity_upd_privilege($userUuid, $uuid, $version);
		if(!$this->permission->success)
		{   
        ...
        {
            echo "<div class='modal-header'><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>×</span></button>This activity has been changed in FLEX from an activity grouping to a single activity. </div>";
			exit;
        }
		...
		if(isset($item_array['teachingTeam']))
		{ 
			if(isset($item_array['teachingTeam']['instructions']))
			{
				$l=0;
			...
			if(isset($item_array['teachingTeam']['linked']))
			{
				$x=0;
				foreach ($item_array['teachingTeam']['linked'] as $linked) 
				{
					...
					foreach ($response['attachments'] as $attachment) 
					{
						...
						{  // uuid match
							...
							{
								case 'file':					
									$item_array['teachingTeam']['linked'][$x]['description'] = $attachment['description'];
									$item_array['teachingTeam']['linked'][$x]['filename'] = $attachment['filename'];
									$item_array['teachingTeam']['linked'][$x]['thumbnail'] = isset($attachment['thumbFilename'])?$attachment['thumbFilename']:'';
									$item_array['teachingTeam']['linked'][$x]['type'] = $attachment['type'];
									break;
								...
								/*case 'linked-resource':
									$item_array['teachingTeam']['linked'][$x]['description'] = $attachment['description'];
									$item_array['teachingTeam']['linked'][$x]['itemUuid'] = $attachment['itemUuid'];
									$item_array['teachingTeam']['linked'][$x]['type'] = $attachment['type'];
									break;*/
								case 'linked-resource':
									if ($attachment['resourceType']	== 'p') {
										$item_array['teachingTeam']['linked'][$x]['pbl'] = 'False';
										$typeasuccess = $this->flexrest->getItem($attachment['itemUuid'], $attachment['itemVersion'], $typearesponse);
										if($typeasuccess)
										{
											if($typearesponse['collection']['uuid']== $pblCollectionUuid)
											{
												$item_array['teachingTeam']['linked'][$x]['pbl'] = 'True';
											}
											
										}					
										$item_array['teachingTeam']['linked'][$x]['description'] = $attachment['description'];
										$item_array['teachingTeam']['linked'][$x]['itemUuid'] = $attachment['itemUuid'];
										$item_array['teachingTeam']['linked'][$x]['itemVersion'] = $attachment['itemVersion'];
										$item_array['teachingTeam']['linked'][$x]['type'] = $attachment['type'];
										$item_array['teachingTeam']['linked'][$x]['resourceType'] = $attachment['resourceType'];	
									}
									if ($attachment['resourceType']	== 'a') {					
										
										$item_array['teachingTeam']['linked'][$x]['itemUuid'] = $attachment['itemUuid'];
										$item_array['teachingTeam']['linked'][$x]['attachmentUuid'] = $attachment['attachmentUuid'];
										$item_array['teachingTeam']['linked'][$x]['itemVersion'] = $attachment['itemVersion'];
										$item_array['teachingTeam']['linked'][$x]['type'] = $attachment['type'];
										$item_array['teachingTeam']['linked'][$x]['resourceType'] = $attachment['resourceType'];
										$typeasuccess = $this->flexrest->getItem($attachment['itemUuid'], $attachment['itemVersion'], $typearesponse);
										...
										if(isset($typearesponse['attachments']))
										{
											...
											foreach($typearesponse['attachments'] as $rAttachment)
											{
												if($rAttachment['uuid'] ==  $attachment['attachmentUuid'])
												{
													$item_array['teachingTeam']['linked'][$x]['itemUuid'] = $typearesponse['uuid'];
													$item_array['teachingTeam']['linked'][$x]['itemVersion'] = $typearesponse['version'];
													$item_array['teachingTeam']['linked'][$x]['description'] = $rAttachment['description'];
													$item_array['teachingTeam']['linked'][$x]['filename'] = $rAttachment['filename'];
													$item_array['teachingTeam']['linked'][$x]['type'] = $rAttachment['type'];
													$item_array['teachingTeam']['linked'][$x]['resourceType'] = $attachment['resourceType'];
								...
								case 'url':
									$item_array['teachingTeam']['linked'][$x]['description'] = $attachment['description'];
									$item_array['teachingTeam']['linked'][$x]['url'] = $attachment['url'];
									$item_array['teachingTeam']['linked'][$x]['type'] = $attachment['type'];
									break;
							}
						}
					}  // end foreach attachment
				} //end of foreach teachingResouce linked
			}
		}
		...
		*/
		...
		echo 'activitylos_array : <pre>';
		print_r($activitylos_array);
		echo '</pre>';*/
		
		//$data = array('item' => $item_array, 'activity_level' =>$activity_level,'token' => $this->generateToken($userUuid), 'privilege' => $user_role);
		$data = array('item' => $item_array, 'activity_level' =>$activity_level,'token' => $this->generateToken($userUuid), 'upd_privilege' => $upd_privilege);
	   /* echo 'data: <pre>';
		print_r($data);
		echo '</pre>';*/
		
		$this->load->view('ocf/flo/lta_item', $data);
  ...
  /**
	...
	// Get activity LOs
		...
		$teachingTeam = '/xml/item/curriculum/activities/activity/teaching_team/detail/uuid';
		
		if($itemXml->nodeValue($teachingTeam) != '') 
		{
			$tmp['teachingTeam']['instructions']['uuid'] = $itemXml->nodeValue($teachingTeam);
		}
		
		if($itemXml->numNodes('/xml/item/curriculum/activities/activity/teaching_team/linked_resources/uuid') > 0)
		{
			$teachingLinked = $itemXml->numNodes('/xml/item/curriculum/activities/activity/teaching_team/linked_resources/uuid');
			if($teachingLinked > 0)
			{
