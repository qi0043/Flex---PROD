...
class Learningandteachingflo extends CI_Controller {
	...
	#echo "<pre>";print_r($upd_privilege);echo "</pre>";exit();
		...
		*/
			...
			foreach ($item_array['activities'] as $linked) {
				...
				foreach ($response['attachments'] as $attachment) 
				{
					...
					if ($linkedUUID == $attachmentUuid) 
					{
						...
						$item_array['activities'][$i]['teachingType'] = $this->attachmentXml2tType($this->$xmlwrapper_name_item);
		...
		echo "<pre>";
		echo "<h2>Unsorted activities</h2>";
		print_r($activities);
		echo "</pre>";
		
		usort($activities, 'sortByType');
		 function sortByType($a, $b) {
   		return strcmp($a['teachingType'], $b['teachingType']);
 }
		echo "<pre>";
		echo "<h2>Sorted activities</h2>";
		print_r($activities);
		echo "</pre>";
  ...
  /**
    ...
    { 
	    ...
	    protected function attachmentXml2tType($attachmentXml) 
    { 
		...
		//$tmp['activityTitle'] = $attachmentXml->nodeValue('/xml/item/itembody/name');
		//$tmp['activityDescription'] = $attachmentXml->nodeValue('/xml/item/itembody/description');
		$tmp = $attachmentXml->nodeValue('/xml/item/curriculum/activities/activity/teaching_types/teaching_type');
		
		 // is this a pre-record?
		 
		 if (($attachmentXml->nodeValue('/xml/item/curriculum/activities/activity/teaching_types/teaching_type') == 'Presentation') && ($attachmentXml->nodeValue('/xml/item/curriculum/activities/activity/online') == 'Online')) {
