	...
	public function index($uuid='missed', $version='missed')
	{
		...
		*/
		...
		foreach ($response['attachments'] as $attachment) {
			...
			//$attachments[$i]['cp'] = 0;
			//$attachments[$i]['ct'] = 0;
			if(stripos($attachment['description'], 'cover photo') > 0) {
			$attachments[$i]['cp'] = stripos($attachment['description'], 'cover photo');
			}
			
			if(stripos($attachment['description'], 'case trigger')> 0) {
			$attachments[$i]['ct'] = stripos($attachment['description'], 'case trigger');
			}
			
			if(stripos($attachment['description'], 'unused')> 0) {
...
/*   --------------------  			
	...
	*/
		...
		$this->load->view('ocf/pblcaseviewFLO', $data);
