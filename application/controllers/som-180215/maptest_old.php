...
class Maptest extends CI_Controller {
	...
	public function index($uuid='missed', $version='missed')
	{
		...
		//$this->logger_topics = $this->logging->get_logger('topics');
      //  $this->logger_topics->error('start to execute' . $time_start);
		$course = array();
		$data = array('course' => $this->recursiveCall(4,'MD','','MMED8','5194ef90-32e1-4d8c-ba14-27dd489c5bf5','TAA', $course, 1));
		...
		$this->logger_topics->error('Controller Maptest executive time: ' . $time);
		$this->logger_topics->error('End of Maptest execute' . $time_end);*/
		$this->load->view('som/cmapTest', $data);   //load data to the view
	}
	...
	**********************************************/
	public function getActivities()
	{
		 ...
		 if($attachmentUuid != '' && $uuid != '')
		 {
			...
			$this->load->view('som/cmap_activities', $returnData); 
		 }
		 else
		 {
			 echo 'not right';
