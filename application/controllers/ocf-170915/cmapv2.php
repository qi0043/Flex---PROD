...
class Cmapv2 extends CI_Controller {
	...
	public function getTopics($courseCode='MD')
	{	
		...
		$course=array();
		$data = array('course' => $this->recursiveCall($course_year, strtoupper($courseCode),'','', $collection_uuid,'TAA', $course, 1));
		unset($course);
...
//		 print_r($activities);
		 ...
		 {	 $course_code = $activities[$x]["course_code"];
			 ...
			 //$courseCode = $activities[$x]["course_code"];
			 $do_condition = '';
		 ...
		 $data = array('data' => $returnData, 'courseCode' => $course_code);
