...
class Workflow extends CI_Controller 
{
	 ...
	 ************************************************************************/
	 public function redraftAndModMilestone($uuid='missed', $version='missed')
	 {
		...
		if(!$success)
		{
			$this->logger_cs->error("Workflow redraft milestone processClientCredentailToekn error:".$this->flexrest->error.'-' . $uuid."/".$version);
			return;
		}
		...
		if(!$success)
		{
			//$this->logger_cs->error("Workflow redraft milestone getItem error:".$this->flexrest->error.'-' . $uuid."/".$version);
			return;
		}
		...
		{
			$success = $this->flexrest->item_redraft($uuid, $version, $response);
			if(!$success)
			{
				//$this->logger_cs->error("Workflow redraft milestone item_redraft error:".$this->flexrest->error.'-' . $uuid."/".$version);
				return;
			}
			
			$success = $this->flexrest->getItem($uuid, $version, $r);
			if(!$success)
			{
				//$this->logger_cs->error("Workflow redraft milestone getItem error:" . $this->flexrest->error. '-' . $uuid."/".$version);
				return;
			...
			if($r['status'] == 'draft')
			{
				$success = $this->flexrest->submitForModeration($uuid, $version, $r);
				if(!$success)
				{
					//$this->logger_cs->error("Workflow redraft milestone submitForModeration error:".$this->flexrest->error. '-' . $uuid."/".$version);
					return;
				}
	...
	public function addAttachment($uuid='missed', $version='missed')
	{
        ...
        $success = $this->flexrest->fileUpload($filearea_uuid, $filename, $testFileContent, $response2);
        if(!$success)
        {
            $errdata['message'] = $this->flexrest->error;
    ...
    public function getComment($uuid='missed', $version='missed')
    {
        ...
        $success = $this->flexrest->getItemHistory($uuid, $version, $response);
        if (!$success) {
            $this->logger_cs->error("Workflow redraft milestone getItem error:" . $this->flexrest->error . '-' . $uuid . "/" . $version);
            return false;
        }
