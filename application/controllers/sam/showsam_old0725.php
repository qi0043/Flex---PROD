...
class Showsam extends CI_Controller 
{
	...
	*/
	...
	#log_message('error', ob_get_level ());
        ...
        $files_exist = $first_files_uuid == null ? true : false;
        $previous_files_exist = $first_previous_files_uuid == null ? true : false;
        
        $count_files = $this->xmlwrapper->numNodes($xpath_files . '/file');
        $count_previous_files = $this->xmlwrapper->numNodes($xpath_previous_files . '/file');
        
        if($files_exist)
        {
            $next_previous_file_uuid = substr($uuid, 0, 33) . '100';
            if($previous_files_exist)
            {
                $next_previous_file_uuid = substr($uuid, 0, 33) . sprintf("%03d", 100+$count_previous_files);
            }
