...
class Showsam extends CI_Controller 
{
    ...
    protected function samXml2Array($itemXml) 
    {
		...
		//get assessment labels subtree
		for ($j = 1; $j <= $itemXml->numNodes('/xml/item/curriculum/assessment/SAMs/labels/label'); $j++) 
        {
			$labelCode = '/xml/item/curriculum/assessment/SAMs/labels/label['.$j.']/code';
			$labelTopicName = '/xml/item/curriculum/assessment/SAMs/labels/label['.$j.']/topic_name';
			$labelPeriod = '/xml/item/curriculum/assessment/SAMs/labels/label['.$j.']/period';
			$labelLocVers = '/xml/item/curriculum/assessment/SAMs/labels/label['.$j.']/loc_vers';
			
			$samsArray['metadata']['labels'][$j]['labelCode'] = $itemXml->nodeValue($labelCode);
			$samsArray['metadata']['labels'][$j]['labelTopicName'] = $itemXml->nodeValue($labelTopicName);
			$samsArray['metadata']['labels'][$j]['labelPeriod'] = $itemXml->nodeValue($labelPeriod);
			$samsArray['metadata']['labels'][$j]['labelLocVers'] = $itemXml->nodeValue($labelLocVers);
		}
