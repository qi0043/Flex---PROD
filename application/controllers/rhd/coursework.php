...
class Coursework extends CI_Controller {
        ...
        public function __construct()
        {
            ...
            if($down_notice != false)
            {
                ...
                if ($down_notice['message'] == '')
                    $down_notice['message'] = 'Coursework thesis submission is temporarily unavailable, please try again later.';
		...
		private function contributorRhd($userUuid)
		{
			...
			if($searchsuccess)
			{
				if(intval($response['available']) > 0)
				{
					...
					$data['name'] =  html_entity_decode($str);
		...
		public function getThesis_part1($uuid='missed', $version='missed', $newThesis = 'missed')
		{
			...
			$str = trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", $item_array['thesis_title'])));
			#$item_array['thesis_title'] =  addslashes(html_entity_decode($str));
			$item_array['thesis_title'] =  html_entity_decode($str);

			$str = trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", $item_array['abstract'])));
			#$item_array['abstract'] =  addslashes(html_entity_decode($str));
			$item_array['abstract'] =  html_entity_decode($str);
			$str = trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", $item_array['keywords'])));
			#$item_array['keywords'] =  addslashes(html_entity_decode($str));
			$item_array['keywords'] =  html_entity_decode($str);
		...
		public function getThesis_part4($uuid='missed', $version='missed', $newThesis = 'missed', $redraft='missed')
		{
			$this->load->helper('url');
			...
			$str = trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", $item_array['embargo_standard_request_reason'])));
			$item_array['embargo_standard_request_reason'] =  html_entity_decode($str);

			$str = trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", $item_array['embargo_extension_request_reason'])));
			$item_array['embargo_extension_request_reason'] =  html_entity_decode($str);


			#Redraft
			if(intval($redraft) == 1 && $response['status'] == 'rejected' && $item_array['stu_fan'] == $_SESSION['fan'])
			{
			    $success = $this->flexrest->item_redraft($uuid, $version, $res_redraft);

			    if(!$success)
			    {
					log_message('error', 'Course work RHD redraft failed, uuid: ' . $uuid);
					$errdata['message'] = 'Internal Error, please try again later!';
					$this->load->view('rhd/showerror_view', $errdata);
					return false;
			    }
		...
		*****************************************************************/
		public function createRHD()
		{
			...
			else
			{
				...
				$abstract = $this->input->post("thesis_abstract");
				...
				else //IF abstract file exists, upload file
				{
					...
					$success = $this->flexrest->fileUpload($filearea_uuid, $abstract_file['name'], file_get_contents($abstract_file['tmp_name']), $r);
					if(!$success)
					{
						$result['status'] = "f_error";
						$errdata['message'] = $this->flexrest->error;
		...
		*****************************************************************/
		...
		public function edit_part1()
		{
			...
			else
			{
				...
				$title =  $this->input->post("thesis_title");
				#The XSS parameter 'TRUE' below causes problem, don't use it
				#$abstract =  htmlentities($this->input->post("thesis_abstract", TRUE));
				$abstract = $this->input->post("thesis_abstract");
				...
				if($this->$xmlwrapper->nodeExists('/xml/item/curriculum/thesis/keywords/keyword'))
				{
					...
					$this->$xmlwrapper->setNodeValue("/xml/item/curriculum/thesis/keywords/keyword", $keywords);
				}
				else
				{
					$keyword = $this->$xmlwrapper->createNodeFromXPath("/xml/item/curriculum/thesis/keywords/keyword");
				...
				//if there is an attachment waiting for upload to xml
				if($default_abstract_file == 'true')
				{
					...
					if(isset($abstract_file['name']) && $abstract_file['name']!= '')
					{
						if($this->$xmlwrapper->nodeExists('/xml/item/curriculum/thesis/version/abstract/uuid'))
						{
							...
							foreach ($new_item_bean['attachments'] as $attachment)
							{
							...
							if(strlen($existing_abstract_item_uuid) == 36 && $existing_file_path!= '')
							{
								...
								if(count($new_item_bean['attachments']) >0)
								{
									...
									foreach($new_item_bean['attachments'] as $a)
									{
										...
										if($f_name == $filename)
										{
											//get actual file name before extension
											$filename = pathinfo($a['filename'], PATHINFO_FILENAME);
											//get file extension
											$file_ext = pathinfo($a['filename'], PATHINFO_EXTENSION);

											if(strlen($filename)>3 && substr($filename, -3) == '('.$count.')' )
											{
											...
											$filename = $filename . '(' . $count . ').' . $file_ext;
										}
									}
								}
								else
								...
								$success = $this->flexrest->fileUpload($filearea_uuid, $filename, file_get_contents($abstract_file['tmp_name']), $r);
								if(!$success)
								{
									$result['status'] = "error";
									$errdata['message'] = $this->flexrest->error;
				...
				else if($default_abstract_file == 'false')
				{
					//IF abstract file already exists. delete it

					if($this->$xmlwrapper->nodeExists('/xml/item/curriculum/thesis/version/abstract/uuid') && $this->$xmlwrapper->nodeValue('/xml/item/curriculum/thesis/version/abstract/uuid')!= '')
					{
						$existing_abstract_uuid = $this->$xmlwrapper->nodeValue('/xml/item/curriculum/thesis/version/abstract/uuid');
					...
					else
					{
						if(isset($_FILES['abstract_file']) && $abstract_file['name']!= '')
						{
							...
							if(count($r_item['attachments']) >0)
							{
		                        ...
		                        /*$result['status'] = "error";
								$errdata['message'] = $this->flexrest->error;
								$result['error_info'] = 'filename: ' . $filename . '$filearea_uuid: ' . $filearea_uuid;
								echo json_encode($result);
								return;*/
							}
							else
				...
				if(!$success)
				{
					$result['status'] = "error";
					$errdata['message'] = $this->flexrest->error;
					log_message('error', '2375. Failed to edit item, item uuid: ' . $item_uuid . ', error: ' . $errdata['message']);
	...
	public function edit_part2_all()
	{
		...
		$thesis_files = isset($_FILES['thesis_file']) ? $_FILES['thesis_file'] : ''; // all new added attachments [FROM THE FILE AREA]
		$attachments = isset($_POST['attachments'])? $_POST['attachments'] : ''; //all attachments including existed attchments and new added attachments. Existied attachments, POST attachment uuid || new added attachments, POST attachment name

		/*if($thesis_files != '')
		{
			for($i=0; $i<count($thesis_files['name']); $i++)
			{
				$result['status'] = 'error';
				$result['error_info'] = "files: " . $thesis_files['name'][$i];
				echo json_encode($result);
				return;
			}
		}*/
		/*if($attachments != '')
		{
			...
			{
				$a = json_decode($attachments[$i], true);
				$str .= $a['uuid'] . $a['file_name'];
			}
			$result['status'] = 'error';
							$result['error_info'] = "attachments: " . $str;
							echo json_encode($result);
			return;
		}*/
		...
		{
			$result['status'] = 'error';
							$result['error_info'] = '2630. There was a problem uploading your file, please contact flex.help@flinders.edu.au for an alternative way to upload the file.';
			echo json_encode($result);
			return;
		}
		...
		if(count($attachments)>0)
		{
			for($i=0; $i<count($attachments); $i++)
			{
				...
				//if attachmet uuid exists, it is an old attachment
				...
				else //if attachment uuid doesn't exist, it is a new attachment, read file from the POST Files
				{
						...
						if($thesis_files!='')
						{
							for($j=0; $j<count($thesis_files['name']); $j++)
							{
								...
								if($thesis_files['name'][$j] == $file_name && $thesis_files['size'][$j] == $file_size)
								{
									...
									if($filearea_uuid=='')
									{
										// if itembean already have attachment existed
										if(count($new_item_bean['attachments']) > 0)
										{
									...
									//check the duplication of attachment names
									$count = 0;
									foreach($new_item_bean['attachments'] as $a)
									{
										$f_name = $a['filename'];
										...
										{
											//get actual file name before extension
											$file_name = pathinfo($a['filename'], PATHINFO_FILENAME);
											//get file extension
											$file_ext = pathinfo($a['filename'], PATHINFO_EXTENSION);

											if(strlen($file_name)>3 && substr($file_name, -3) == '('.$count.')' )
											{
									...
									//$success = $this->flexrest->fileUpload($filearea_uuid, $thesis_files['name'][$j], file_get_contents($thesis_files['tmp_name'][$j]), $r);
									$success = $this->flexrest->fileUpload($filearea_uuid, $file_name, file_get_contents($thesis_files['tmp_name'][$j]), $r);
									//usleep(100000);
	...
	public function edit_part3()
	{
		...
		if($release_status == 'Restricted Access')
		{
			...
			$embargo_reason = isset($_POST['embargo_reason']) ? $_POST['embargo_reason'] :'' ;
			...
			if($embargo_duration == '36'|| $embargo_duration == 36)
			{
				...
				if($embargo_ext_request == 'Additional Restriction')
				{
					...
					$embargo_ext_reason = isset($_POST['embargo_ext_reason']) ? $_POST['embargo_ext_reason']:'' ;
		...
		private function itemXml2Array($itemXml)
		{
			...
			$stu_fan = '/xml/item/curriculum/people/students/student/fan';
			$tmp['stu_fan'] =  addslashes(html_entity_decode($itemXml->nodeValue($stu_fan)));
		    ...
		    $embargo_extension = '/xml/item/curriculum/thesis/release/embargo_request/extension_request/requested';
			$tmp['embargo_extension'] = $itemXml->nodeValue($embargo_extension);
		...
		private function thesis_validation($item_array, $attachments)
		{
			...
			else if($item_array['open_access_required'] == 'new version')
			{
				...
				if(isset($item_array['open_access']))
				{
					...
					if(count($item_array['open_access']) < 1)
					{
			...
			if($release_status == 'Restricted Access')
			{
				...
				if(intval($embargo_standard_request_duration) ==  36)
				{
					...
					if($embargo_extension == 'Additional Restriction')
					{
