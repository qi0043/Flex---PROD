...
?>
  ...
  <?php foreach($_SESSION['ocf_validgrouplist'] as $key=>$value) {
    ...
    <div class="col-md-3 col-sm-12 col-xs-12"> <a href="#" onClick="javascript:logoutMsg();" class="btn btn-default btn-xs"><span class="small"><i class="fa fa-power-off"></i> <span class="text-uppercase">log out</span></span></a> </div>
  </div>
</div>
...
<div role="main">
  ...
  <div id="myNav" class="span10"><a href="<?php echo base_url() . 'ocf/home/' . $courses['code'] ?>" class="btn btn-sm btn-primary">Return to dashboard</a>&nbsp;&nbsp; <?php if (!(empty($coursestring))) { ?><a href="/flex/ocf/<?php echo strtolower($courses['code']) ; ?>/fmgo" class="btn btn-sm btn-success"  data-toggle="tooltip" data-placement="top" title="<?php echo $coursestringlong; ?>"><?php echo $coursestring; ?></a><?php } ?></div>

<h4>Learning Outcome Map - <?php echo $profstringlong; ?></h4>
...
<a href="/flex/ocf/<?php echo $courses['code']; ?>/amcgo/0" class="btn <?php if ($theLevel == 0) { ?>active<?php } ?> btn-sm btn-default" style=" float:left;">Show None</a>
</div>

<div class="btn-group" role="group" style="margin-bottom:10px; float:left;">
<a href="/flex/ocf/<?php echo $courses['code']; ?>/amcgo/1" class="btn <?php if ($theLevel == 1) { ?>active<?php } ?> 
 btn-sm btn-default" >Level 1</a>
<a href="/flex/ocf/<?php echo $courses['code']; ?>/amcgo/2" class="btn <?php if ($theLevel == 2) { ?>active<?php } ?> 
 btn-sm btn-default" >Level 2</a>
<a href="/flex/ocf/<?php echo $courses['code']; ?>/amcgo/3" class="btn <?php if ($theLevel == 3) { ?>active<?php } ?> 
 btn-sm btn-default" >Level 3</a>
</div>


<div class="btn-group" role="group" style="margin-bottom:10px; float:left;">
<a href="/flex/ocf/<?php echo $courses['code']; ?>/amcgo" class="btn btn-sm btn-default" >Show All</a>
</div>
</div>
...
<div class="span12" id="theData">
<?php if($courses['numProfLO'] > 42) { ?>
...
<div role="tabpanel">
  ...
  <?php foreach($profdomain as $domain) { ?>
    ...
    <li role="presentation" class="text-<?php echo $domain['class']; ?>"><a href="#<?php echo $domain['code']; ?>" aria-controls="<?php echo $domain['code']; ?>" role="tab" data-toggle="tab"><?php echo $domain['code']; ?></a></li>
...
<?php } ?>
 ...
 <table class="table-bordered table-responsive" style="margin-bottom:5em; margin-top:2em;">
   ...
   <thead>
    ...
    <th colspan="<?php echo $domain['number']; ?>" valign="bottom" class="<?php echo $domain['class']; ?>" style="padding:5px;" scope="col"><?php echo $domain['code']; ?></th>
 ...
 <?php } ?>  
   ...
   <?php foreach ($topics as $topic) { ?>
    <tr>
      ...
      <?php for ($i = $domain['start']; $i <= ($domain['start'] + $domain['number'] - 1); $i++ ) { ?>
      <td align="center" style="padding:5px;" <?php if($topic['prof']['los']['lo'.$i]['numAlign'] > 0) { ?> class="notNullBG"<?php } ?>>
      
      
      
<?php if ($topic['prof']['los']['lo'.$i]['level'] == $topiccount['theLevel']) { ?>

<span class="notNullBG" style="padding:5px;"><img src="<?php echo base_url() . 'resource/flo/ocf/';?>images/flextra-level-<?php echo $topic['prof']['los']['lo'.$i]['level']; ?>.svg" alt="level<?php echo $topic['prof']['los']['lo'.$i]['level']; ?>" width="18" height="18" data-toggle="tooltip" data-placement="top" title="Level <?php echo $topic['prof']['los']['lo'.$i]['level']; ?>"></span>
<?php } else { ?>
<span class="notNullBG" style="padding:5px;"><img src="<?php echo base_url() . 'resource/flo/ocf/images/blank.gif'; ?>" alt="" width="18" height="18"></span><?php } ?></td>
   <?php } ?>  
    </tr>
   ...
   <table class="table-bordered " style="margin-bottom:5em;">
     <thead>
       ...
       <tr>
         ...
         <th valign="bottom" style="padding:5px; width:4.2em;" scope="col"><a href="../amcgosearch/<?php echo strtolower($topics[1]['prof']['los']['lo'.$i]['code']);?>"><?php echo $topics[1]['prof']['los']['lo'.$i]['code'];?></a>
           <?php if (strlen($topics[1]['prof']['los']['lo'.$i]['code']) < 4) { ?>
           &nbsp;
           <?php } ?></th>
         <?php } ?>
       </tr>
     ...
     <tbody>
       <?php foreach ($topics as $topic) { ?>
       <tr>
         <td style="padding:5px; padding-right:3em;"><a href="topic/<?php echo $topic['uuid']; ?>/<?php echo $topic['version']; ?>" title="<?php echo $topic['title']; ?>"><?php echo $topic['code']; ?></a></td>
         <?php for ($i = 1; $i <= $courses['numProfLO']; $i++ ) { ?>
         <td align="center" style="padding:5px;" <?php if($topic['prof']['los']['lo'.$i]['numAlign'] > 0) { ?> class="notNullBG1"<?php } ?>><?php if ($topic['prof']['los']['lo'.$i]['level'] == $topiccount['theLevel']) { ?>
           <span class="notNullBG" style="padding:5px;"><img src="<?php echo base_url() . 'resource/flo/ocf/';?>images/flextra-level-<?php echo $topic['prof']['los']['lo'.$i]['level']; ?>.svg" alt="level<?php echo $topic['prof']['los']['lo'.$i]['level']; ?>" width="18" height="18" data-toggle="tooltip" data-placement="top" title="Level <?php echo $topic['prof']['los']['lo'.$i]['level']; ?>"></span>
           <?php } else { ?>
           <span class="notNullBG" style="padding:5px;"><img src="<?php echo base_url() . 'resource/flo/ocf/images/blank.gif'; ?>" alt="" width="18" height="18"></span>
           <?php } ?></td>
         <?php } ?>
       </tr>
       <?php } ?>
