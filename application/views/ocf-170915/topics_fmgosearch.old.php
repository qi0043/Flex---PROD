...
<div class="container-fluid" style="margin:0 20px 0 20px;">
  <div role="main">
    ...
    <div class="row">
      <div id="myNav" class="span10"><a href="/flex/ocf/startup" class="btn btn-sm btn-primary">Return to dashboard</a>&nbsp;&nbsp;<a href="/flex/ocf/fmgo" class="btn btn-sm btn-success"> Flinders Medical Graduate Outcomes</a>&nbsp;&nbsp;<a href="/flex/ocf/amcgo" class="btn btn-sm btn-success">AMC Graduate Outcomes</a></div>
      <h3>Flinders Medical Graduate Outcome  :: <?php echo $topics[1]['catName']; ?> - <?php echo $topics[1]['locode']; ?></h3>
      <p><?php echo $topics[1]['loName']; ?></p>
    </div>
    <div class="span10">
      <h4>Topics</h4>
      <table class="table-bordered" style="margin-bottom:2em; width:96%;">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td align="center" valign="top" style="padding:5px; width: 22em;" scope="col"><?php if ($topic['level'] == 0 && $topic['course']['lo']['numAlign'] > 0) { ?>
              <span class="btn btn-xs  btn-danger heatmap" data-toggle="tooltip" data-placement="top" data-html="true" title="<strong>Warning: data mis-match!</strong><br />Level not identified with associated Learning Outcomes"><i class="fa fa-exclamation-triangle"></i></span>
              <?php } ?>
              <?php if($topic['level'] != '' && $topic['level'] >= 0) { ?>
              <img src="/flex/resource/ocf/images/heat<?php echo $topic['level']; ?>.png" alt="" width="32" height="32" class="heatmap" title="Level <?php echo $topic['level']; ?>"<?php if($topic['level'] > 0) { ?> data-toggle="tooltip" data-placement="top" <?php } ?>>
              <?php } else { ?>
              &nbsp;
              <?php } ?></td>
            <td align="left" valign="top" style="padding:5px; width: 22em;" scope="col"><?php if(isset($topic['course']['loaligned'])) { ?>
              <?php foreach($topic['course']['loaligned'] as $aligned) { ?>
              <strong><?php echo $aligned['code']; ?></strong> :: <?php echo $aligned['name']; ?><br />
              <?php }  // end foreach   ?>
              &nbsp;
              <?php }  // ecn isset   ?>
              <?php if ($topic['level'] > 0 && $topic['course']['lo']['numAlign'] == 0) { ?>
